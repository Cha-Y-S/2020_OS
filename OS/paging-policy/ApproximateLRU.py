...
if options.solve == False:
    ...
    if policy=='CLOCK':
        print ('*: Use bit of page is 1     not *: Use bit of page is 0')
        print ('')
    ...
    count = 0
    memory = []
    clock_hand = 0  # clock hand
    hits = 0
    miss = 0

    if policy == 'CLOCK':
        for i in range(cachesize):  #각 frame을 x(empty space의 의미)로 init
            memory.append('x')
    ...
    for nStr in addrList:
        ...
        if idx == -1:
            ...
            if count == cachesize:
                # 현재 clock frame에 들어가 있는 VPN의 개수와 cache size가 같은 경우
                # 반드시 replace가 발생함
                if policy == 'FIFO' or policy == 'LRU':
                    ...
                    victim = -1
                    # replace의 대상이 될 victim을 찾는 과정
                    while victim == -1:
                        # 현재 clock_hand가 가리키고 있는 frame의 use bit를 검사
                        page = memory[clock_hand]
                        if cdebug:
                            print('  scan page:', page, ref[page])

                        if ref[page] >= 1:
                            # clock_hand가 가리키고 있는 frame의 use bit가 1일 경우 0으로 함
                            # Nth chance인 경우는 count를 세주지만, 현재 구현한 clock algorithm은 한 번의 기회밖에 없음
                            ref[page] -= 1
                        else:
                            # this is our victim
                            # clock_hand가 가리키고 있는 frame의 use bit가 0일 경우 
                            # 해당 page가 교체 대상이 되고 잠시 x로 교체하여 빈 공간 형성
                            victim = page
                            memory[clock_hand] = 'x'
                            break
                        # victim을 찾지 못한경우 clock_hand를 조정하여 victim을 찾을 때 까지 반복
                ...
                # cache frame이 아직 full이 되지 않을 경우 replace를 하지 않음
                # -> count != cachesize인 경우
                victim = -1
                count = count + 1

            # clock_hand가 가리키는 곳(현재는 'x')에 VPN insert
            # 이후 clock_hand가 다음 frame을 가리키도록 조정
            if policy == 'CLOCK':
                memory[clock_hand] = n
                clock_hand = (clock_hand + 1) % cachesize
            else:
        ...
        # VPN이 이미 clock frame에 있는 경우는 clock_hand의 조정 없이 use bit만 바꾸어 줌
        # after miss processing, update reference bit
        if n not in ref:
            ref[n] = 1
        else:
            if policy == 'CLOCK':
                ref[n] = 1      # not Nth chance
            else:
                ref[n] += 1
            if ref[n] > clockbits:
                ref[n] = clockbits
        ...
        # VPN reference 후 결과 출력
        if (notrace == False) and (policy == 'CLOCK'):
            print ('Access: %d  %s %s -> ' % (n, hfunc(idx), leftStr,),end="")
            print ('%5s' %('['),end="")
            for i in range (len(memory)):
                if memory[i] == 'x':
                    print('%s ' %(memory[i]),end="")
                elif ref[memory[i]] == 1:
                    print('%d*' %(memory[i]),end="")
                else:
                    print('%d ' %(memory[i]),end="")
                if(i != len(memory)-1):
                    print(', ',end="")
            print('%-2s\t' %(']'),end="")
            print ('<- %s Replaced:%s [Hits:%d Misses:%d] Clock hand: %d' % (riteStr, vfunc(victim), hits, miss, clock_hand))
        elif notrace == False:
