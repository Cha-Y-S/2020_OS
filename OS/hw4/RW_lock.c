...
void *worker(void *arg) {
    ...
    for (i = 0; i < args->arrival_delay; i++) {
        //TICK;
        space(args->thread_id); printf("ad %d of %d\n", i, args->arrival_delay); space_end();
    }
...
void *reader(void *arg) {
   ...
   // start reading
    ...
    for (i = 0; i < args->running_time-1; i++) {
        TICK;
        space(args->thread_id); printf("r %d of %d\n", i, args->running_time); space_end();
    }
    //TICK;
    space(args->thread_id); printf("r %d of %d, DB is %d\n", i, args->running_time, DB); space_end();
    // end reading
    //TICK;
...
void *writer(void *arg) {
   ...
   // start writing
    ...
    for (i = 0; i < args->running_time-1; i++) {
        //TICK;
        space(args->thread_id); printf("w %d of %d\n", i, args->running_time); space_end();
    }
    //TICK;
    DB++;
    space(args->thread_id); printf("w %d of %d, DB is %d\n", i, args->running_time, DB); space_end();
    // end writing 
    //TICK;
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5)
    {
        printf("%s: use -n (number of workers) -a (job_type:arrival_delay:running_time)\n",argv[0]);
        exit(1);
    }
    ...
    for (i = 0; i < num_workers; i++) {
        a[i].thread_id = i;
        if(i == 0)
            a[i].job_type = atoi(strtok(argv[4]," :,\n"));
        else
            a[i].job_type = atoi(strtok(NULL," :,\n"));
        a[i].arrival_delay = atoi(strtok(NULL, " :,\n"));
        a[i].running_time = atoi(strtok(NULL, " :,\n"));
        /* printf("%d workers %d:%d:%d\n",
            a[i].thread_id,
            a[i].job_type,
            a[i].arrival_delay,
            a[i].running_time); */
